...
関数呼び出し 関数の宣言は{\bf func}から始まり，{\bf return}で終わるよう
     ...
     返り値とする．関数の呼び出しは呼び出し名$\clabel{n}$と関数名および引数を\{\}
     で囲って記述する．
\end{itemize}
...
される変数$x$は$\blabel{1}.\blabel{2}.\plabel{1}.x$のように名前付けを行い以降はこの名前で扱う．
...
\hspace*{.25\linewidth}$(B_i+1,B_i,w,\rho,\xi,\sigma)$\newline
$(E-1,P',w,\delta,\bot,\rho,\xi,\sigma)\brightarrow{(\mathsf{par},1)}_{p\cdot i}$\\
\hspace*{.25\linewidth}$(E,E-1,w,\rho,\xi,\sigma)$\newline
\bcode{par}\ 0における並列ブロックのラベル$\alabel{n}$，動的コンテクスト$\delta$は，親プロセス$p$から
受け渡される．$(B_i',E_i')=T(\alabel{n})(i)$に対して$B_i=N+1-B'_i$, $E_i=N+1-E'_i$として，
\bcode{r\_fork}命令によって起動される．$B_i$からから\bcode{par}\ 0を実行し，$E_i-1$において
\bcode{par}\ 1を実行する．
%\item \bcode{par}:\\
...
\figref{fig:sample}を順方向実行のバイトコードに変換すると\figref{fig:bytecode}になる．
...
\ecaption{a target program(\texttt{bug\_fact})}
\label{fig:target}
\end{figure}
...
\figref{fig:backwardfact}の逆方向実行バイトコードと\figref{fig:value}，\figref{fig:label}の
逆方向実行に必要な情報を用いて順方向の実行を逆方向に辿る．\figref{fig:value}と
