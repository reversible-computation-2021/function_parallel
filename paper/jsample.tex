...
\figref{fig:jmp}はジャンプ履歴の保存と利用法について示している．プロセスpの抽象機械が$SM_p$がパス$\pi$でジャンプしlabel命令を行うと\figref{fig:jmp}のようにラベルスタックにジャンプしてきたPCの値とそれを行ったプロセス番号が保存される．
...
\figref{fig:store}には変数更新履歴の保存と利用方法を示している．\figref{fig:store}の$\sigma$は共有変数スタックを表し，$w$は演算スタックを表す．\figref{fig:store}のようにstore命令を行うと失われるはずの変数の更新前の値をstore命令を行ったプロセス番号とパスとともに値スタックに保存する．
